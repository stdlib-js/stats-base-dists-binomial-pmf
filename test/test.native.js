...
tape( 'if provided `NaN` for `x` or `p`, the function returns `NaN`', opts, function test( t ) {
...
tape( 'if `p` or `n` equals `0`, the function evaluates a degenerate distribution centered at `0.0`', opts, function test( t ) {
	var y;

	// Case: n = 8, p = 0.0
	y = pmf( 0.0, 8, 0.0 );
	t.strictEqual( y, 1.0, 'returns 1 for x equal to 0' );
...
tape( 'if `p` equals `1.0`, the function evaluates a degenerate distribution centered at `n`', opts, function test( t ) {
	var y;

	y = pmf( 8.0, 8, 1.0 );
	t.strictEqual( y, 1.0, 'returns 1 for x equal to 8' );
